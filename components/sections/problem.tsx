...
export function Problem() {
  return (
    <section id="service" className="py-24 sm:py-36">
      <Container>
        <Reveal>
          <SectionHeading eyebrow={problem.eyebrow} headline={problem.headline} size="lg" />
        </Reveal>

        {/* Scattered — everything a store owner has to keep in their head.
            Each card settles into place on its own, one after another. */}
        <div className="mt-16 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5">
          {problem.tasks.map((task, i) => {
            const Icon = icons[task.icon];
            return (
              <Reveal key={task.label} delay={120 + i * 80}>
                <div
        ...
        {/* Resolved — it all lands in one place, once the scatter has settled. */}
        <Reveal delay={120 + problem.tasks.length * 80 + 200}>
